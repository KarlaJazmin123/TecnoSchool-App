...
class Analisis_hipotesisE : AppCompatActivity() {
    @SuppressLint("MissingInflatedId")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_analisis_hipotesis_e)

        val btnRegresar2 = findViewById<Button>(R.id.btnRegresar2)
        btnRegresar2.setOnClickListener {
            val intent = Intent(this, Graficos_visualizacionesE::class.java)
            startActivity(intent)
            finish()
        }
        val btnSiguiente = findViewById<Button>(R.id.btnSiguiente)
        btnSiguiente.setOnClickListener {
            val intent = Intent(this, Analisis_estadistico_financieroE::class.java)
            startActivity(intent)
            finish()
        }
