...
class Funciones_TextoE : AppCompatActivity() {
    @SuppressLint("MissingInflatedId")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_funciones_texto_e)

        val btnRegresar2 = findViewById<Button>(R.id.btnRegresar2)
        btnRegresar2.setOnClickListener {
            val intent = Intent(this, Funciones_Basicas_Excel::class.java)
            startActivity(intent)
            finish()
        }
        val btnSiguiente = findViewById<Button>(R.id.btnSiguiente)
        btnSiguiente.setOnClickListener {
            val intent = Intent(this, Formato_CeldasE::class.java)
            startActivity(intent)
            finish()
        }
